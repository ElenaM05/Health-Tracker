...
function parseIngredientString(ingredientStr) {
  ...
  if (ingredientStr.toLowerCase().includes('to taste') || 
    ...
    return {
      name: ingredientStr.toLowerCase().replace(/\s*(to taste|as needed|for seasoning)\s*/i, '').trim(),
      amount: 0,
      unit: 'to taste',
      original: ingredientStr,
      originalAmount: 0,
      originalUnit: 'to taste',
      skipNutrition: true // Flag to skip nutrition calculation
    };
  }
...
async function addNutritionalData(ingredients) {
  ...
  for (const ingredient of ingredients) {
    try {
      // Skip nutrition calculation for "to taste" ingredients
      if (ingredient.skipNutrition) {
        results.push({
          ...ingredient,
          nutrition: {
            calories: 0,
