...
import { 
  Calculator, List, Edit3, Link, Save, X, Globe, ChefHat, Users, Clock, 
  Eye, Check, AlertCircle, Loader2, Plus
} from 'lucide-react';

export default function CalorieTracker({ currentView: externalView }) {
  // Use external view if provided, otherwise use internal navigation
  const hasExternalNav = externalView !== undefined;
  const [internalView, setInternalView] = useState('input');
  ...
  const handleUrlSubmit = async (e) => {
    ...
    try {
      ...
      const parseResponse = await fetch('/api/parse-recipe-url', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
      ...
      // Check if nutrition data was already found
      ...
      <div className="max-w-6xl mx-auto px-4">
        <div className="flex justify-between items-center h-16">
          ...
          <div className="flex space-x-1">
            ...
            <button
              onClick={() => setInternalView('recipes')}
              className={`flex items-center gap-2 px-4 py-2 rounded-lg font-medium transition-all duration-200 ${
                currentView === 'recipes'
                  ? 'bg-green-500 text-white shadow-lg'
                  : 'text-green-700 hover:bg-green-100'
    ...
    <div className="w-full max-w-6xl mx-auto px-4 py-10">
      ...
      {!recipesLoading && !recipesError && savedRecipes.length === 0 && (
        <div className="text-center py-12">
          ...
          <button
            onClick={() => hasExternalNav ? null : setInternalView('input')}
              ...
              <div className="mt-4 flex gap-2">
                <button
                  onClick={() => {
                    ...
                    if (!hasExternalNav) setInternalView('input');
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-emerald-100 via-emerald-200 to-teal-300">
      ...
      <main>
        {(currentView === 'input' || (!hasExternalNav && currentView === 'input')) && <RecipeInputView />}
        {currentView === 'recipes' && <RecipesListView />}
      </main>
